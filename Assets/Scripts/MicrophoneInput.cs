...
public class MicrophoneInput : MonoBehaviour {
	...
	//MicrophoneInput micI = gameObject.AddComponent(typeof(SphereCollider)) as SphereCollider;

	private List<string> options = new List<string>();
	private int samples = 8192; 
	private AudioSource audioSource;

	public void InitMircophone(){
	...
	public void StartMicrophone(){
		...
		if (Microphone.IsRecording (microphone)) { //check that the mic is recording, 
			//otherwise you'll get stuck in an infinite loop waiting for it to start
